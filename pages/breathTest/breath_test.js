    ...
    app.globalData.localSocket.onMessage(function(res) {
      ...
      if(res.code==1 && res.type==1){
        ...
        that.setData({
          ...
          tip:"呼吸频率：" + app.fomatFloat(res.data.breath_rate,1)
        })
      }else if(res.code==0 && res.type==1 && app.fomatFloat(res.time,1) > 3){
        wx.request({
          url: HOST + '/breath/stop',
          method:"GET",
          header:{
            'content-type':'application/x-www-form-urlencoded'
          },success:res=>{
            wx.request({
              url: HOST + '/breath/collect',
              method:"GET",
              header:{
                'content-type':'application/x-www-form-urlencoded'
              },success:res=>{
